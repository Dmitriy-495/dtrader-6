...
// публикует bot в market:orderbook:{symbol} (см. gateway.OrderBookUpdate).
//
// ВАЖНО (см. проектирование): на текущий момент это поле в Redis
// содержит ПОСЛЕДНЮЮ ИНКРЕМЕНТАЛЬНУЮ ДЕЛЬТУ стакана, а не полный
// снапшот — bot дорабатывается параллельно, чтобы отдавать полный
// снапшот с той же структурой полей. OrderBookReader уже написан под
// целевой (полный) формат — как только bot начнёт публиковать полный
// стакан по этому же ключу и с этими же именами полей, никаких
// изменений в reader/orderbook.go не потребуется.
